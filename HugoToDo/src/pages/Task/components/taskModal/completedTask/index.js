...
const CompletedTask = ({
  state: {
    todos: { current, showModalCompletedTask },
  },
  hideModalCompletedTask,
}) => (
  <Modal animationType="slide" transparent={false} visible={showModalCompletedTask}>
    <View style={style.container}>
      <TouchableOpacity
        activeOpacity={0.8}
        onPress={() => hideModalCompletedTask()}
      ...
      {current && (
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <View style={{ flexDirection: 'row' }}>
              <Text style={{ color: '#989898', fontSize: 10, marginRight: 10 }}>
                Completed Date
              </Text>
              <Text style={{ color: 'black', fontSize: 14 }}>{current.completedDate}</Text>
            </View>
          </View>
        </View>
      )}
